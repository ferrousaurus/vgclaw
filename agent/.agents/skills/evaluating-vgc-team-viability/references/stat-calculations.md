...
Where:
- `base` = base HP stat via `assets/pokemon.json`
- `SP` = Stat Points (0-32)
- Individual Values (IVs) are always set to 31 in Pokemon Champions, and the level is always 50. This is already factored into the simplified formula.
...
```
stat = floor((base + SP + 20) * natureMultiplier)
```

Where:
- `base` = base stat (Atk, Def, SpA, SpD, or Spe) via `assets/pokemon.json`
- `SP` = Stat Points (0-32)
...
Use these procedures to determine exact speed benchmarks — how many SPs to outspeed a target, effective speed under modifiers, and team speed tier lists.
...
To find the minimum Speed SPs needed to outspeed a target:

1. Calculate the target's Speed stat at their assumed investment
2. Your Pokemon needs a Speed stat of at least `target + 1`
3. Solve backwards for SPs: rearrange the stat formula to find the minimum SPs that produce the required stat

**Solving for SPs given a target stat:**

```
requiredStat = targetSpeed + 1
baseStatNeeded = ceil(requiredStat / natureMultiplier)
SPs = baseStatNeeded - base - 20
```

If SPs < 0, 0 SPs is enough. If SPs > 32, the Pokemon cannot outspeed the target with that nature — try a Speed-boosting nature or speed control.
...
```
requiredStat = 85
baseStatNeeded = ceil(85 / 1.0) = 85   (Adamant doesn't affect Spe)
SPs = 85 - 78 - 20 = -13 → negative means 0 SPs is enough
```

Check: Feraligatr with 0 Spe SPs, Adamant:
```
stat  = 78 + 0 + 20 = 98
final = floor(98 * 1.0) = 98
```

Under Tailwind: `98 * 2 = 196`. 196 > 169. Confirmed: **Adamant Feraligatr with 0 Speed SPs outspeeds max Speed Jolly Garchomp under Tailwind.**
...
| Choice Scarf | 1.5x | `assets/items.json` `speedModifier` field |
...
1. For each team member, calculate their Speed stat at their current SPs/nature using the formula above
2. Look up base Speed values of common threats via `assets/pokemon.json`
...
Where:
- `power` = move's base power via `assets/moves.json`
...
- 0.75x if the move's `target` field via `assets/moves.json` is "All Adjacent Foes" or "All Adjacent" (hits both opponents in doubles)
...
- 1.5x if the move's type matches one of the attacker's types (via `assets/pokemon.json`)
...
- **2HKO with Sitrus Berry:** `damage * 2 >= defender's HP stat + floor(defender's HP stat * hpRestore)` where `hpRestore` is from the defender's item via `assets/items.json` (Sitrus Berry: 0.25)
...
**Step 2 — Defense stat:**
Incineroar: base HP 95, base Def 90. 32 HP SPs, 0 Def SPs, assume neutral nature.
```
HP = 95 + 32 + 75 = 202
Def = floor((90 + 0 + 20) * 1.0) = 110
```
...
When damage exceeds HP, you need more bulk. Two options: more HP SPs or more Def/SpD SPs.

**Finding minimum HP SPs (with current Def/SpD):**

Calculate the damage at the current Def/SpD. You need `HP > damage`. Solve the HP formula for SPs:

```
requiredHP = damage + 1
SPs = requiredHP - baseHP - 75
```

If SPs > 32, HP investment alone won't save you — add Def/SpD SPs too.

**Finding minimum Def/SpD SPs (with current HP):**
...
To reduce baseDamage, increase defStat. Find the minimum defStat that makes final damage (after all modifiers) less than HP, then solve backwards for SPs using the stat formula.

### HP vs. Defense Investment Efficiency

**General rule:** HP SPs benefit both physical and special bulk. Def/SpD SPs only help against one side.
...
**Procedure to compare:** For a specific attack, calculate the damage reduction from +1 HP SP vs. +1 Def SP. Whichever reduces the damage-to-HP-ratio more is the more efficient investment for that specific matchup.

### Multi-Hit Benchmarking

For 2HKO survival:
- Without recovery: you survive if `damage * 2 < HP`
- With Sitrus Berry: you survive if `damage * 2 < HP + floor(HP * 0.25)`. (Sitrus Berry `hpRestore: 0.25` via `assets/items.json` heals 25% of max HP when HP drops below 50%)
...
With 32 HP SPs: HP = `85 + 32 + 75` = 192. 120 vs 192 = survives (62.5%). More comfortable margin.
...
1. For each candidate, calculate their offensive stat at 32 SPs with a boosting nature
...
Solving for SPs: Adamant Feraligatr (base Atk 105, nature 1.1x)
```
baseStatNeeded = ceil(130 / 1.1) = ceil(118.18) = 119
SPs = 119 - 105 - 20 = -6 → negative
```

Negative means even 0 Atk SPs with Adamant is enough! Let's verify:

0 Atk SPs Adamant Feraligatr: `floor((105 + 0 + 20) * 1.1)` = `floor(125 * 1.1)` = `floor(137.5)` = 137.
